...
4. Create shortcut to prog and move it to the desktop
...
### On Linux:
Sadly, but there is no Linux binaries out currently. But you can compile the prog from the source code. Exact compilation steps are specified below.
...
#### On Linux:
On Linux compilation process is mostly the same. But with an exception for compiling helper class and some parts of the C++ source code.
You will need to manually compile the class in the terminal (.sh builder coming soon) and you will need to modify parts of the C++ code, that use Windows specific functions (e.g. extract_icon)
...
1. Go back to the [Releases](https://github.com) and try another version.
...
### "Folder in use" error when trying to delete prog directory:
1. Open **Task Manager**.
2. Go to the **Details** tab.
3. Find `adb.exe` in the list.
4. Kill the process.

> **info:** This happens because the Android Debug Bridge (ADB), which the app uses to connect to your phone, runs as a background daemon. It stays active so that any other app trying to run an ADB command can do so instantly, without waiting for a lengthy ADB initialization.
